...
`ISSUES.md` at the repo root and append to it as you go:
...
Commit and push the fixes as you make them — docs/SELF_ITERATION.md is the
standing rule for that, and it applies to this session too.
...
- **Netlist to board** — solved during the original run and shipped as
  `kh board-from-netlist`; KiCad still exposes no headless path of its own.
  Left here because it is the shape of gap worth attacking: read ISSUES.md for
  what is still open.
