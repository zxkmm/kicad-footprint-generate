...
The running log of things that broke, surprised, or were missing while actually
using these skills. Append to it — that is the point. Anything you fix in the
same session belongs in the code or the docs instead; this file is for what you
hit and did **not** fix. See [`docs/SELF_ITERATION.md`](docs/SELF_ITERATION.md).

The first batch was kept while building a TP4056 USB-C charger end to end with
nothing but this harness — library lookup, schematic authored as text, netlist,
board, placement, routing, checks. Everything here was hit in practice, not read
off the source.
